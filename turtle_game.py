...
class Text():
    def __init__(self, x=0, y=0, txtColor='black', txtFillcolor='green', text='hello', ffont='Arial', fsize=8,
                 ftype='normal'):
        # setup:
        penup()
        hideturtle()
...
def isTailTouching(curX, curY):
    i = 1
    while i < len(snake):
        if (curX + step / 2 > snake[i].xcor() > curX - step / 2 and
                curY + step / 2 > snake[i].ycor() > curY - step / 2):
            return True
        i += 1
    return False
